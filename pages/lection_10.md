...
exportFilename: pdf/oop_lection_10
layout: cover
title: oop_10
mdc: true
---

# Объектно-ориентированное программирование<br>Лекция 10: Порождающие паттерны

Factory Method (Фабричный метод). Abstract Factory (Абстрактная фабрика). Builder (Строитель). Prototype (Прототип). Singleton (Одиночка).
...
```python {*|5-11|7,11|13-14|16}{maxHeight: '420px'}
class PizzaBuilder:
    def __init__(self):
        self.pizza = []
    ...
    def add_pepperoni(self):
        self.pizza.append("pepperoni")
        return self

    def build(self):
        return f"Pizza with {', '.join(self.pizza)}"

my_lunch = PizzaBuilder().add_cheese().add_pepperoni().build()
```

---

# Строитель (`Builder`).<br>Результаты:
...
```python {*|4-8|31-32|10-17|31-39|19-28|24|25-27|28|41-51}{maxHeight: '420px'}
import copy
from typing import List

class Disk:
    """Вспомогательный класс: Жесткий диск"""
    def __init__(self, size_gb: int, content: List[str]):
        self.size_gb = size_gb
        self.content = content

class VirtualMachine:
    """Сложный объект, который мы будем клонировать"""
    def __init__(self, name: str, cpu: int, ram: int, disk: Disk, ip: str):
        self.name = name
        self.cpu = cpu
        self.ram = ram
        self.disk = disk
    ...
    def clone(self):
        """
        Создает полную копию (deep copy) виртуальной машины.
        Важно использовать deepcopy, так как Disk — это изменяемый объект.
        """
        new_vm = copy.deepcopy(self)
        # При клонировании мы можем сразу сбросить уникальные данные
        new_vm.name += "_clone"
        new_vm.ip = "0.0.0.0" # Требует новой настройки
        return new_vm


# 1. Долгая и сложная настройка "Золотого образа"
boot_disk = Disk(size_gb=50, content=["OS_Linux", "App_Server", "Configs"])
golden_image = VirtualMachine(
    "Ubuntu-Base",
    cpu=4,
    ram=16,
    disk=boot_disk,
...
# 2. Массовое развертывание (Клонирование)
# Нам не нужно заново указывать CPU, RAM и содержимое диска
worker_node_1 = golden_image.clone()
worker_node_1.name = "Worker-01"
worker_node_1.ip = "192.168.1.101"
# Изменим диск клона, чтобы доказать независимость объектов
worker_node_1.disk.content.append("Worker_Logs.txt")

worker_node_2 = golden_image.clone()
worker_node_2.name = "Worker-02"
worker_node_2.ip = "192.168.1.102"
```
